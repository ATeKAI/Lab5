        ...
        private void SelectingPerformingOperation(Operator op)
        {
            try
            {
                ...
                else if (op.symbolOperator == 'N')
                {
                    ...
                    {
                        Rectangle figure = null;
                        string new_name = operands.Pop().value.ToString();
                        string old_name = operands.Pop().value.ToString();
                        foreach (Figure f in FiguresContainer.figureList)
                        {
                            if (f.name == old_name)
                            {
                        ...
                        if (figure != null)
                        {
                            ...
                            log.Items.Add($"Фигура {old_name} успешно переименована в " +
                                          $"{figure.name}");
                        }
                        else
                        {
                            MessageBox.Show($"Фигуры {old_name} не существует");
                            log.Items.Add($"Фигуры {old_name} не существует");
                        }
                        
                    }
                    else
                    {
                        MessageBox.Show("Опертор N принимает 2 параматетра.");
                        log.Items.Add("Неверное число параметров для оператора N.");
                    }
                }
            }
            catch (Exception ex)
